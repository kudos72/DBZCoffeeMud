...
public class StdMOB implements MOB
{
	...
	private volatile double		freeActions		= 1.0;
	...
	{
		return ((int) Math.round(CMath.div(charStats().getStat(CharStats.STAT_QUICKNESS) - 8, 4.0)) + 1);
	}
	...
	public void recoverCharStats()
	{
		...
		if ((playerStats != null) && (soulMate == null) && (playerStats.getHygiene() >= PlayerStats.HYGIENE_DELIMIT))
		{
			...
			if ((charStats.getStat(CharStats.STAT_QUICKNESS) / 2) > chaAdjust)
				charStats.setStat(CharStats.STAT_QUICKNESS, charStats.getStat(CharStats.STAT_QUICKNESS) - chaAdjust);
			else
				charStats.setStat(CharStats.STAT_QUICKNESS, charStats.getStat(CharStats.STAT_QUICKNESS) / 2);
		}
	}
		...
		if (tickID == Tickable.TICKID_MOB)
		{
			...
			if (location() != null)
			{
				...
				else
				{
					...
					//setActions(actions() + (charStats().getStat(CharStats.STAT_INTELLIGENCE) /20) + (charStats().getStat(CharStats.STAT_DEXTERITY) /20) + (charStats().getStat(CharStats.STAT_QUICKNESS) / 20));
