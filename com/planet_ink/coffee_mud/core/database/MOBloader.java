...
public class MOBloader
{
	...
	public MOB DBReadUserOnly(final String name, final String[] locationID)
	{
		...
		try
		{
			...
			if(R.next())
			{
				...
				stats.setStat(CharStats.STAT_ENDURANCE,CMath.s_int(DBConnections.getRes(R,"CMCONS")));
				stats.setStat(CharStats.STAT_GENDER,DBConnections.getRes(R,"CMGEND").charAt(0));
				stats.setStat(CharStats.STAT_SPIRIT,CMath.s_int(DBConnections.getRes(R,"CMWISD")));
				stats.setStat(CharStats.STAT_INTELLIGENCE,CMath.s_int(DBConnections.getRes(R,"CMINTE")));
				stats.setStat(CharStats.STAT_QUICKNESS,CMath.s_int(DBConnections.getRes(R,"CMCHAR")));
	...
	public void DBUpdateJustMOB(MOB mob)
	{
		...
		DB.updateWithClobs(
				...
				+", CMCONS="+mob.baseCharStats().getStat(CharStats.STAT_ENDURANCE)
				+", CMGEND='"+((char)mob.baseCharStats().getStat(CharStats.STAT_GENDER))+"'"
				+", CMWISD="+mob.baseCharStats().getStat(CharStats.STAT_SPIRIT)
				+", CMINTE="+mob.baseCharStats().getStat(CharStats.STAT_INTELLIGENCE)
				+", CMCHAR="+mob.baseCharStats().getStat(CharStats.STAT_QUICKNESS)
