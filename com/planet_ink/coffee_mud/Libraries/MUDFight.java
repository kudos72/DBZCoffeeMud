	...
	public void recoverTick(MOB mob)
	{
		...
		&&(!CMLib.flags().isClimbing(mob)))
		{
			...
			final double[] vals=new double[]{
				charStats.getStat(CharStats.STAT_ENDURANCE),
				mob.phyStats().level(),
			...
			vals[0]=((charStats.getStat(CharStats.STAT_INTELLIGENCE)+charStats.getStat(CharStats.STAT_SPIRIT)));
	...
	public void tickCombat(MOB fighter)
	{
		...
		if(CMLib.flags().aliveAwakeMobile(fighter,true))
		{
			...
			||(fighter.isMonster()))
			{
				...
				if(CMLib.dice().rollPercentage()>(fighter.charStats().getStat(CharStats.STAT_ENDURANCE)*4))
