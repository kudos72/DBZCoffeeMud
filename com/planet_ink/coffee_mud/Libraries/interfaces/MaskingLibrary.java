...
public interface MaskingLibrary extends CMLibrary
{
	...
	public final String DEFAULT_MASK_HELP =
		...
		+"+SUBOP (allow local-room area staff to bypass the rules)  <BR>"
		+"-SUBOP (always <WORD> local-room area staff)  <BR>"
		...
		+"-SECURITY (<WORD> all security flags, even a lack of a security flag) <BR>"
		+"-SECURITY +cmdrooms +area cmditems etc..  (create exceptions)<BR>"
		+"+SECURITY (do not <WORD> any or no security flags) <BR>"
		+"+SECURITY -cmdrooms +area cmditems, etc.. (create exceptions) <BR>"
		...
		+"+NAMES -bob \"-my name\" etc.. (create name exceptions) <BR>"
		+"-CLAN (<WORD> everyone, even no clan) <BR>"
		...
		+"-SKILLFLAG (<WORD> everyone, even those with no skills) <BR>"
