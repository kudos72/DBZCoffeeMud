		...
		for(int v=0;v<V.size();v++)
		{
			...
			if(zapCodes.containsKey(str))
				...
				case 5: // -Levels
					{
						for(int v2=v+1;v2<V.size();v2++)
							buf.append(levelHelp(V.elementAt(v2),'+',L(skipFirstWord?"Only ":"Allows only ")));
					}
					break;
				case 6: // -ClassLevels
					{
						for(int v2=v+1;v2<V.size();v2++)
							buf.append(levelHelp(V.elementAt(v2),'+',L(skipFirstWord?"Only class ":"Allows only class ")));
					}
					break;
				case 103: // -MaxclassLevels
					{
						for(int v2=v+1;v2<V.size();v2++)
							buf.append(levelHelp(V.elementAt(v2),'+',L(skipFirstWord?"Only highest class ":"Allows only highest class ")));
					}
					break;
				case 7: // -Tattoos
					{
			...
			case 105: // -weather
				...
				case 33: // +Item
					{
						buf.append(L((skipFirstWord?"The":"Requires the")+" following item"+(multipleQuals(V,v,"-")?"s":"")+": "));
						for(int v2=v+1;v2<V.size();v2++)
						{
							final String str2=V.elementAt(v2);
							if(zapCodes.containsKey(str2))
				...
				case 58: // +WornOn
					{
						...
						for(int v2=v+1;v2<V.size();v2++)
						{
							...
							if((str2.startsWith("-"))||(str2.startsWith("+")))
							{
								final long code=Wearable.CODES.FIND_endsWith(str2.substring(1));
								if(code>=0)
									parms.addElement(Long.valueOf(code));
							}
							v=V.size();
						}
						buf.add(new CompiledZapperMaskEntry(entryType.intValue(),parms.toArray(new Object[0])));
				...
				case 70: // -Disposition
					{
						...
						for(int v2=v+1;v2<V.size();v2++)
						{
							...
							if((str2.startsWith("-"))||(str2.startsWith("+")))
							{
								final int code=CMLib.flags().getDispositionCode(str2.substring(1));
								if(code>=0)
									parms.addElement(Integer.valueOf((int)CMath.pow(2,code)));
							}
							v=V.size();
						}
						buf.add(new CompiledZapperMaskEntry(entryType.intValue(),parms.toArray(new Object[0])));
	...
	public boolean maskCheck(final CompiledZapperMask cset, final Environmental E, final boolean actual)
	{
		...
		for(final CompiledZapperMaskEntry entry : cset.entries)
		{
			...
			case 20: // +adjint
				...
				break;
			case 21: // +adjwis
				if((entry.parms.length>0)&&(mob.charStats().getStat(CharStats.STAT_SPIRIT)<(((Integer)entry.parms[0]).intValue())))
				   return false;
				break;
			case 22: // +adjdex
				...
				break;
			case 23: // -adjcha
				if((entry.parms.length>0)&&(mob.charStats().getStat(CharStats.STAT_ENDURANCE)<(((Integer)entry.parms[0]).intValue())))
				   return false;
				break;
			case 24: // +adjcha
				if((entry.parms.length>0)&&(mob.charStats().getStat(CharStats.STAT_QUICKNESS)<(((Integer)entry.parms[0]).intValue())))
				   return false;
				break;
			case 25: // -adjstr
				if((entry.parms.length>0)&&(mob.charStats().getStat(CharStats.STAT_STRENGTH)>(((Integer)entry.parms[0]).intValue())))
			...
			case 89: // +wis
				base=getBaseCharStats(base,mob);
				if((entry.parms.length>0)&&(base.getStat(CharStats.STAT_SPIRIT)<(((Integer)entry.parms[0]).intValue())))
				   return false;
				break;
			case 90: // +dex
				base=getBaseCharStats(base,mob);
			...
			case 91: // +con
				base=getBaseCharStats(base,mob);
				if((entry.parms.length>0)&&(base.getStat(CharStats.STAT_ENDURANCE)<(((Integer)entry.parms[0]).intValue())))
				   return false;
				break;
			case 92: // +cha
				base=getBaseCharStats(base,mob);
				if((entry.parms.length>0)&&(base.getStat(CharStats.STAT_QUICKNESS)<(((Integer)entry.parms[0]).intValue())))
				   return false;
				break;
			case 93: // -str
				base=getBaseCharStats(base,mob);
