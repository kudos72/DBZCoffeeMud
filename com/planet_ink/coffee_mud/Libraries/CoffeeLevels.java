...
public class CoffeeLevels extends StdLibrary implements ExpLevelLibrary
{
	...
	public int getManaBonusNextLevel(MOB mob)
	{
		...
		final double[] variables={
				mob.phyStats().level(),
				mob.charStats().getStat(CharStats.STAT_SPIRIT),
				CMProps.getIntVar(CMProps.Int.BASEMAXSTAT)+mob.charStats().getStat(CharStats.CODES.toMAXBASE(CharStats.STAT_SPIRIT)),
				...
				mob.charStats().getStat(CharStats.STAT_QUICKNESS),
				mob.charStats().getStat(CharStats.STAT_ENDURANCE)
			};
	...
	public int getMoveBonusNextLevel(MOB mob)
	{
		...
		final double[] variables={
			...
			mob.charStats().getStat(CharStats.STAT_ENDURANCE),
			CMProps.getIntVar(CMProps.Int.BASEMAXSTAT)+mob.charStats().getStat(CharStats.CODES.toMAXBASE(CharStats.STAT_ENDURANCE)),
			mob.charStats().getStat(CharStats.STAT_SPIRIT),
			mob.charStats().getStat(CharStats.STAT_INTELLIGENCE)
		};
	...
	public int calcMaxHitPoints(MOB mob)
	{
		final CharClass charClass = mob.baseCharStats().getCurrentClass();
		final double[] variables={
			mob.phyStats().level(),
		...
		final int newHitPointTotal=(int)Math.round(CMath.parseMathExpression(charClass.getHitPointsFormula(), variables));
		if(newHitPointTotal<=0)
		{
			if(mob.charStats().getStat(CharStats.STAT_ENDURANCE)>=1)
				return 1;
			return 0;
		}
		return newHitPointTotal;
	}
	...
	public int getPlayerHitPoints(MOB mob)
	{
		...
		return calcMaxHitPoints(mob);
//		return hp+((mob.phyStats().level()-1)*calcMaxHitPoints(mob));
	}
	...
	public StringBuffer baseLevelAdjuster(MOB mob, int adjuster)
	{
		...
		final int newHitPointGain = calcMaxHitPoints(mob) * adjuster;
		if(mob.getWimpHitPoint() > 0)
		{
	...
	public void unLevel(MOB mob)
	{
		...
		int prac2Stat=mob.charStats().getStat(CharStats.STAT_SPIRIT);
		final int maxPrac2Stat=(CMProps.getIntVar(CMProps.Int.BASEMAXSTAT)
					 +mob.charStats().getStat(CharStats.CODES.toMAXBASE(CharStats.STAT_SPIRIT)));
