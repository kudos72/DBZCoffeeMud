...
public class CMGenEditor extends StdLibrary implements GenericEditor
{
		...
		throws IOException
	{
		...
		while((mob.session()!=null)&&(!mob.session().isStopped())&&(!c.equals("\n")))
		{
			...
			case 'C': toggleClimateMask(A,Places.CLIMASK_COLD); break;
			case 'H': toggleClimateMask(A,Places.CLIMASK_HOT); break;
			case 'R': toggleClimateMask(A,Places.CLIMASK_WET); break;
			case 'W': toggleClimateMask(A,Places.CLIMASK_WINDY); break;
			case 'D': toggleClimateMask(A,Places.CLIMASK_DRY); break;
			}
		}
	}
		...
		while((mob.session()!=null)&&(!mob.session().isStopped())&&(!q))
		{
			...
			else
			{
				...
				if(newValue>=0)
				{
					...
					{
						final StringBuilder str=new StringBuilder(CMParms.toStringList(me.getBreathables()));
						if(str.length()>0)
			...
			if((showFlag<=0)||(showFlag==showNumber))
			{
				...
				if((showFlag==showNumber)||(showFlag<=-999))
				{
					...
					while((mob.session()!=null)&&(!mob.session().isStopped()))
					{
						...
						{
							final StringBuilder str=new StringBuilder(CMParms.toStringList(RawMaterial.Material.values()));
	...
	throws IOException
	{
		...
		final String allComponents=CMParms.toStringList(RawMaterial.Material.values())+","+CMParms.toStringList(RawMaterial.CODES.NAMES());
		while((mob.session()!=null)&&(!mob.session().isStopped())&&(!ok))
		{
			int showNumber=0;
	...
	throws IOException
	{
		...
		while((mob.session()!=null)&&(!mob.session().isStopped()))
		{
			...
			||(!CMLib.law().doesOwnThisLand(C.clanID(),newRoom)))
