	...
	protected void doThang()
	{
			...
			if(ellapsed>=myRace.getAgingChart()[1])
			{
				...
				if(R!=null)
				{
					...
					else
					{
						...
						babe.baseCharStats().setStat(CharStats.STAT_ENDURANCE,7);
						...
						babe.baseCharStats().setStat(CharStats.STAT_SPIRIT,2);
		...
		&&(((MOB)affected).location().isInhabitant(((MOB)affected).amFollowing())))
		{
			...
			&&(babe.fetchBehavior("MudChat")==null))
			{
				...
				if(R!=null)
				{
					...
					babe.baseCharStats().setStat(CharStats.STAT_QUICKNESS,10);
					babe.baseCharStats().setStat(CharStats.STAT_ENDURANCE,10);
					...
					babe.baseCharStats().setStat(CharStats.STAT_SPIRIT,6);
					if(following!=null)
		...
		&&(CMLib.law().getLandOwnerName(((MOB)affected).location()).length()>0))
		{
			...
			if(ellapsed>=myRace.getAgingChart()[3])
			{
				...
				babe.baseCharStats().setStat(CharStats.STAT_SPIRIT,10);
