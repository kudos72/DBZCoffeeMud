...
public class Spell_MindLight extends Spell
{
	...
	public boolean tick(Tickable ticking, int tickID)
	{
		...
		for(int m=0;m<R.numInhabitants();m++)
		{
			...
			{
				if(invoker()!=null)
					M.curState().adjMana((invoker().charStats().getStat(CharStats.STAT_INTELLIGENCE)+invoker().charStats().getStat(CharStats.STAT_SPIRIT))/8,M.maxState());
				else
					M.curState().adjMana((M.charStats().getStat(CharStats.STAT_INTELLIGENCE)+M.charStats().getStat(CharStats.STAT_SPIRIT))/8,M.maxState());
			}
		}
		return true;
	}
