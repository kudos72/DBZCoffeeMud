	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget, boolean auto, int asLevel)
	{
		...
		if(success)
		{
			...
			if(mob.location().okMessage(mob,msg))
			{
				...
				if(target.charStats().getStat(CharStats.STAT_SPIRIT)>=18)
					adjective+="incredibly wise, ";
				else
				if(target.charStats().getStat(CharStats.STAT_SPIRIT)>=13)
					adjective+="very wise, ";
				else
				if(target.charStats().getStat(CharStats.STAT_SPIRIT)>10)
					adjective+="wise, ";
				else
				if(target.charStats().getStat(CharStats.STAT_SPIRIT)<=3)
					adjective+="extremely naieve, ";
				else
				if(target.charStats().getStat(CharStats.STAT_SPIRIT)<7)
					adjective+="naieve, ";
				else
				if(target.charStats().getStat(CharStats.STAT_SPIRIT)<10)
					adjective+="unwise, ";
