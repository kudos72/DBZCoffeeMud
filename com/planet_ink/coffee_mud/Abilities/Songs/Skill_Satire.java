...
public class Skill_Satire extends BardSkill
{
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget, boolean auto, int asLevel)
	{
		...
		if(!success)
		{
			...
			if(CMLib.dice().rollPercentage()>mob.charStats().getStat(CharStats.STAT_QUICKNESS))
		...
		if(mob.location().okMessage(mob,msg))
		{
			...
			if((msg.value()>0)||(CMLib.dice().rollPercentage()<(25-mob.charStats().getStat(CharStats.STAT_QUICKNESS))))
			{
				criminalFail(B,A2,mob,target);
			}
		}
