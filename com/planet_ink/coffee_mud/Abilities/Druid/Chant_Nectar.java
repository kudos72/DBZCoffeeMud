			...
			case CMMsg.TYP_DRINK:
				{
					...
					final int hp=CMLib.dice().roll(1,M.charStats().getStat(CharStats.STAT_ENDURANCE)+super.getX1Level(invoker())+super.getXLEVELLevel(invoker()),0);
					CMLib.combat().postHealing(M,M,this,CMMsg.MASK_ALWAYS|CMMsg.TYP_CAST_SPELL,hp,null);
					final int mana=CMLib.dice().roll(1,((M.charStats().getStat(CharStats.STAT_SPIRIT)+M.charStats().getStat(CharStats.STAT_INTELLIGENCE))/2)+super.getX1Level(invoker())+super.getXLEVELLevel(invoker()),0);
					M.curState().adjMana(mana,M.maxState());
					final int move=CMLib.dice().roll(1,((M.charStats().getStat(CharStats.STAT_SPIRIT)+M.charStats().getStat(CharStats.STAT_INTELLIGENCE))/2)+super.getX1Level(invoker())+super.getXLEVELLevel(invoker()),0);
