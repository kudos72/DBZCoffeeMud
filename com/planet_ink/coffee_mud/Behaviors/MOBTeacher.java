...
public class MOBTeacher extends CombatAbilities
{
	...
	protected void ensureCharClass()
	{
		...
		myMOB.baseCharStats().setStat(CharStats.STAT_SPIRIT,19);
		...
		&&(!CMSecurity.isDisabled(CMSecurity.DisFlag.MOBTEACHER)))
		{
			...
			if(x>=0)
			{
				...
				if(giveABonus)
				{
					...
					monster.baseCharStats().setStat(CharStats.STAT_SPIRIT,25);
				...
				monster.baseCharStats().setStat(CharStats.STAT_SPIRIT,19);
				monster.recoverCharStats();
			}
		}
