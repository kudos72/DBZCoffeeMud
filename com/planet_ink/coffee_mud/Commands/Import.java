...
public class Import extends StdCommand
{
	...
	throws IOException
	{
		...
		for(final Enumeration e=files.keys();e.hasMoreElements();)
		{
			...
			final String data=(String)files.get(filename);
			...
			if(new CMFile(filename,mob).exists())
			{
				...
				if(!noPrompt)
					if(!mob.session().confirm(L("\n\rExternal resource '@x1' found, import (Y/n)?",filename),L("Y")))
						continue;
			}
	...
	throws IOException
	{
		...
		for(int c=0;c<custom.size();c++)
		{
			if(custom.get(c) instanceof Race)
			{
				...
				{
					if(!noPrompt)
						if(!mob.session().confirm(L("Custom Race '@x1' found, import (Y/n)?",R.ID()),L("Y")))
							continue;
					CMClass.addRace(R);
				...
				if(!R2.isGeneric())
				{
					...
					if(!noPrompt)
						if(!mob.session().confirm(L("Custom Race '@x1' found which would override your standard race.  Import anyway (Y/n)?",R.ID()),L("Y")))
							continue;
					CMClass.addRace(R);
					CMLib.database().DBCreateRace(R.ID(),R.racialParms());
				}
			...
			if(custom.get(c) instanceof CharClass)
			{
				...
				{
					if(!noPrompt)
						if(!mob.session().confirm(L("Custom Char Class '@x1' found, import (Y/n)?",C.ID()),L("Y")))
							continue;
				...
				if(!C2.isGeneric())
				{
					...
					if(!noPrompt)
						if(!mob.session().confirm(L("Custom Char Class '@x1' found which would override your standard class.  Import anyway (Y/n)?",C.ID()),L("Y")))
							continue;
					CMClass.addCharClass(C);
					CMLib.database().DBCreateClass(C.ID(),C.classParms());
				}
			...
			if(custom.get(c) instanceof Ability)
			{
				...
				{
					if(!noPrompt)
						if(!mob.session().confirm(L("Custom Ability '@x1' found, import (Y/n)?",A.ID()),L("Y")))
							continue;
				...
				if(!A2.isGeneric())
				{
					...
					if(!noPrompt)
						if(!mob.session().confirm(L("Custom Ability '@x1' found which would override your standard Ability.  Import anyway (Y/n)?",A.ID()),L("Y")))
							continue;
			...
			if(custom.get(c) instanceof Manufacturer)
			{
				...
				{
					if(!noPrompt)
						if(!mob.session().confirm(L("Custom Manufacturer '@x1' found, import (Y/n)?",M.name()),L("Y")))
							continue;
				...
				else
				{
					...
					if(!noPrompt)
						if(!mob.session().confirm(L("Custom Manufacturer '@x1' found which would override your existing one.  Import anyway (Y/n)?",M.name()),L("Y")))
							continue;
					eM.setXml(M.getXml());
				}
			}
	...
	{
		final Vector helpsToEat=new Vector();
								...
								Vector commands)
	{
		if(OfThisID.startsWith("#"))
		{
			...
			{
				final MOB M=(MOB)((MOB)areaMOBS.get(OfThisID.substring(1))).copyOf();
			...
			{
				final MOB M=(MOB)((MOB)doneMOBS.get(OfThisID.substring(1))).copyOf();
				M.setStartRoom(putInRoom);
				M.setLocation(putInRoom);
				return M;
			}
		...
		else
		{
			...
			{
				final MOB M=(MOB)((MOB)areaMOBS.get(OfThisID)).copyOf();
				M.setStartRoom(putInRoom);
				M.setLocation(putInRoom);
				return M;
			}
			...
			{
				final MOB M=(MOB)((MOB)doneMOBS.get(OfThisID)).copyOf();
				M.setStartRoom(putInRoom);
				M.setLocation(putInRoom);
				return M;
			}
		...
		throws java.io.IOException
	{
		...
		final Hashtable doneItems=new Hashtable();
		final Hashtable doneRooms=new Hashtable();
		final Hashtable doneMOBS=new Hashtable();
		final Vector nextResetData=new Vector();
		final Hashtable laterLinks=new Hashtable();
		boolean multiArea=false;
		final List<CMObject> custom=new Vector<CMObject>();
		final Hashtable externalFiles=new Hashtable();
		...
		final Vector resetData=new Vector();
		...
		final Vector newRooms=new Vector();
		final Vector socialData=new Vector();
		...
		try
		{
			...
			if((buf!=null)&&(buf.length()>20)&&(buf.substring(0,20).indexOf("<AREAS>")>=0))
			{
				...
				final List<List<XMLLibrary.XMLpiece>> areas=new Vector();
				if(session!=null)
		...
		try
		{
			...
			Room lastRoom=null;
			final Hashtable petShops=new Hashtable();
			final Hashtable<String,Room> areaRooms=new Hashtable<String,Room>();
			final Hashtable areaItems=new Hashtable();
			final Hashtable areaMOBS=new Hashtable();
				...
				while(roomV.size()>0)
				{
					...
					if(nextLine.toUpperCase().startsWith("D"))
					{
						...
						if(((linkRoom==null)||(linkRoom.getArea().Name()!=R.getArea().Name()))&&(linkRoomID>=0))
						{
							try
							{
								for(final Enumeration r2=CMLib.map().rooms();r2.hasMoreElements();)
								{
			...
			for(final Enumeration e=petShops.keys();e.hasMoreElements();)
			{
				final Room storeRoom=(Room)e.nextElement();
				final Room shopRoom=(Room)petShops.get(storeRoom);
		...
		{
			for(final Enumeration e=laterLinks.keys();e.hasMoreElements();)
			{
				final String key=(String)e.nextElement();
				final String dcode=(String)laterLinks.get(key);
				String roomID="";
				String dirID="";
